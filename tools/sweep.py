...
and a control whose value never reaches the encoder is dead just as quietly:
everything compiles, links, loads and renders. Nothing else in this repo
catches that.

This renders each parameter at both ends of its range against the same
moving test card and reports any that made no difference at all.

    python3 tools/sweep.py [--binary build/txtest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card moves, and the page arrives by the field.** Every render is forty
host frames in (field 33 at 50 Hz), so Rows per Field has had fields to tear
in and Freeze has had a page to hold -- Freeze from frame 0 holds the empty
page, which is black, against a picture.

**Page Number needs the header**, which is on by default. Show Codes needs
control cells, which every row has (column 0 at least).

**Hold Graphics and Separated need mosaics next to control cells**; the card
has colour boundaries mid-row, so it has them.

**An option's range is its element count, an integer's is its real range.**
`txtest --list` prints both for exactly this reason, and the ends are what
get swept.
...
CONTEXT = {
    # The header is on by default, so the page number shows; said explicitly.
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "txtest"))
    ...
    scratch = tempfile.mkdtemp(prefix="txsweep")
