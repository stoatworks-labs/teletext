...
`github.com/stoatworks-labs/teletext`, released at v0.1.0 on 2026-09-24 with a user
...
| `demo/` | The browser demo: `plugin.js` holds the plugin's three shader bodies verbatim and a hand PORT of the CPU half (decoder, encoder, transmission, header, layout, controls, the font table); `tools/check_shaders.py` keeps the shaders identical; `vendor/` is the shared kit from `stoatworks-backend/resolume-demo` (never edit it, re-run `sync.sh`). Served by this repo's own Worker at `teletext-demo.stoatworks-labs.com` through a DNS record + route (the zone is out of custom domains). |
...
- **No presets, no OpenFX port.** `StoatworksAbout.h` and `ATTRIBUTIONS.md` are
  generated by the backend's `sync-about.py` and `sync-attributions.py`; the user guide
  is `docs/USER-GUIDE.md`, rendered to the site and `docs/USER-GUIDE.pdf` by the
  website's `build_guides.py`.
- **The browser demo's integer controls are dropdowns** (the kit has no integer type),
  and its CPU half omits the `Perturb` hooks, the greedy encoder and the forced-error
  hook, which are harness-only. Its costs live in doubles: a row's cost is under 2^28,
  so the integers are the same integers.
...
- **Never loaded into Resolume on macOS.** Everything numeric was compiled, rendered
  and measured offline against the real plugin class in a headless CGL context, plus an
  `oxbow` load. How the read-back stall behaves inside a busy host, and what the host's
  clock does over hours, is untested.
- **Windows, in Resolume Arena 7.27.1** (win-lab, Mesa llvmpipe, no GPU, 2026-09-24):
  the v0.1.0 DLL loads from Extra Effects, registers as `SW Teletext` / `TX01` / effect,
  all 18 host controls match the declaration, it renders and Arena's log stays clean:
  8 of the fleet gate's 9 checks, twice. Rows per Field and Freeze read dead both
  times, because the gate's carrier is a still: a still page comes round in under a
  second at any Rows per Field, and a frozen still is the same still. Software
  rendering says nothing about a GPU or about speed.
- **Footage has only been seen through `--pipe`** (the project video, on Resolume's
  bundled demo clips): a dark clip loses its mid-tones to black, as eight colours must.
  Nothing in a host has shown footage.
