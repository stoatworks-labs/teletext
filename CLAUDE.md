...
Local v0.1.0, unreleased; never loaded into Resolume.
...
## Not done yet
- **Never loaded into Resolume.** Everything is measured offline on macOS, plus an
  `oxbow` load. No Windows build has been made (CI cannot run yet).
- Seen on the synthetic card only; never on footage.
- No OpenFX port, no browser demo, no factory presets, no user guide
  (`StoatworksAbout.h` has `guide=""` and is a provisional hand copy, as is
  `ATTRIBUTIONS.md`).
- Not registered on the website; no release tag.
